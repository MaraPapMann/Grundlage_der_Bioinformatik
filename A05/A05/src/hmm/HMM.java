...
/**
 * A simple A.HMM class
 * Huajie Chen 4199962 & Anastasia Grekova
...
public class HMM {
    ...
    private String generatingPath = ""; // State path used to generate symbols

    // I. Constructors:

    /**
     * Construct an empty A.HMM.
     */
    public HMM() {
        this(0, 0);
    }
     ...
     * Construct an empty A.HMM.
     *
     * @param numStates  the number of states that the A.HMM will hold
     * @param numSymbols the number of symbols that the A.HMM will hold
     */
    ...
    /**
     * Construct an A.HMM from a Reader
     *
     * 这一大段其实都是在读取hmm description 然后套入到上面设置的各种数列/矩阵里。必须要结合文档格式一起看才能理解。
     * @param r the Reader
     */
    public HMM(Reader r) throws Exception {
        this(0, 0);
        StreamTokenizer st = new StreamTokenizer(r); //这里使用了StreamTokenizer, 详见https://blog.csdn.net/yin380697242/article/details/51987834 主要是分割字符串
        st.commentChar('#');
        ...
        try {
            ...
            if (st.nextToken() != StreamTokenizer.TT_EOF && st.ttype == StreamTokenizer.TT_NUMBER && st.nval > 0)
                numStates = (int) st.nval; //如果条件满足，那么numStates=当下token的数值
            else throw new
    ...
    /**
     * Read an A.HMM from a string.
     *
     * @param str the string containing a description of the A.HMM
     */
    ...
    /**
     * Print a description of the A.HMM to a string.
     *
     * @return a description of the A.HMM
     */
    public String toString() { // 这里是把采集到的hmm信息作成一个String反馈出来
    ...
    // III. Methods for querying the A.HMM (is and get functions):

    /**
     * Check that the A.HMM contains valid and consistent data, throw an
     * exception, if this is not the case
     */
    public void checkValid() throws Exception { //检查这个HMM是否达标
    ...
    // V. Computational methods and algorithms:

    /**
     * Chooses the next state using the transition matrix
     *
     * @param k current state
     * @return next state
     */
    public int chooseNextState(int k) { //选择下一个骰子状态
        double p=(double)(0+Math.random()*1);
        if(k==0){
            k=(int)(1+Math.random()*(numStates-1));
        }else{
            for(int i=1;i<numStates;i++){
                if(p<=transMatrix[i][0]){
                    k=0;
                }else{
                    k=(int)(1+Math.random()*(numStates-1));
                }
    ...
    /**
     * Chooses a symbol to emit
     *
     * @param k the current state
     * @return the number of the emitted symbol
     */
    public int emitSymbol(int k) { //产出一个数字
        int e=0;
        if(k==0){
            e=-1;
        }else{
            e=(int)(0+Math.random()*numSymbols);
        }
        return e;
    }

    /**
     * Randomly generate a sequence of symbols using the HMM,
     * the state path used to generate the symbols can be accessed using
     * getGeneratingPath().
     *
     * @return the sequence of emitted symbols
     */
    public String generateSequence() {
        StringBuilder symbols = new StringBuilder(); // the emitted symbols
        StringBuilder path = new StringBuilder(); // the state path used

        int k = chooseNextState(0);
        int b= emitSymbol(k);
        while(k!=0 && b!=-1){
            char p1=getStateName(k);
            char s1=getSymbolName(b);
            path.append(p1);
            symbols.append(s1);
            k=chooseNextState(k);
        ...
        generatingPath = path.toString(); // access via getGeneratingPath
        return symbols.toString();
    }

    /**
     * Get the state path used to generate the sequence of symbols returned
     * by generateSequence()
     *
     * @return the state path used
     */
    public String getGeneratingPath() {
        return generatingPath;
    }
        ...
        for (int i = 1; i <= L; i++) {
            ...
            for (int l = 0; l < numStates; l++) {
                ...
                for (int k = 0; k < numStates; k++) {
                    VA = v[k][i - 1] * transMatrix[k][l];
                    if (VA > maxVA) {
                        maxVA = VA;
                        ptr[i][l] = k;
                    }
                }
                v[l][i] = emissionProb[l][xi] * maxVA;
                //System.out.println("V"+"l: "+l+"i: "+i +"is "+v[l][i]);
                //System.out.println("ptr[i][l]: "+ptr[i][l]);
            }
        }
            ...
            int maxPtr = 0;
            for (int k = 0; k < numStates; k++) {
                double Value = v[k][L] * transMatrix[k][0];
                if (Value > maxValue) {
                    maxValue = Value;
                    maxPtr = k;
                }
